...
test("navigating a frame with Boost.visit", async ({ page }) => {
  const pathname = "/src/tests/fixtures/frames/frame.html"

  await page.locator("#frame").evaluate((frame) => frame.setAttribute("disabled", ""))
  await page.evaluate((pathname) => window.Boost.visit(pathname, { frame: "frame" }), pathname)
...
test("following a link preserves the current <boost-frame> element's attributes", async ({ page }) => {
  const currentPath = pathname(page.url())

  await page.click("#hello a")

  const frame = page.locator("boost-frame#frame")
...
test("following a link sets the frame element's [src]", async ({ page }) => {
  ...
  const { url } = await nextEventOnTarget(page, "frame", "boost:before-fetch-request")
...
test("following a link doesn't set the frame element's [src] if the link has [data-boost-stream]", async ({ page }) => {
  ...
  expect(originalSrc, "the boost-frame src should not change after clicking the link").toEqual(newSrc)
})
...
test("following an a[data-boost-frame=_top] does not toggle the frame's [aria-busy=true] attribute", async ({
  page
}) => {
...
test("submitting a form[data-boost-frame=_top] does not toggle the frame's [aria-busy=true] attribute", async ({
  page
}) => {
...
test("successfully following a link to a page without a matching frame dispatches a boost:frame-missing event", async ({
  page
}) => {
  await page.click("#missing-frame-link")
  const { response } = await nextEventOnTarget(page, "missing", "boost:frame-missing")
...
test("successfully following a link to a page without a matching frame shows an error and throws an exception", async ({
  ...
  expect(error.message).toContain(`The response (200) did not contain the expected <boost-frame id="missing">`)
})

test("successfully following a link to a page with `boost-visit-control` `reload` performs a full page reload", async ({
  page
}) => {
...
test("failing to follow a link to a page without a matching frame dispatches a boost:frame-missing event", async ({
  page
}) => {
...
test("failing to follow a link to a page without a matching frame shows an error and throws an exception", async ({
  ...
  expect(error).toBeTruthy()
  expect(error.message).toContain(`The response (404) did not contain the expected <boost-frame id="missing">`)
})

test("the boost:frame-missing event following a link to a page without a matching frame can be handled", async ({
  page
}) => {
  ...
  await page.click("#missing-frame-link")
  await nextEventOnTarget(page, "missing", "boost:frame-missing")

  await expect(page.locator("#missing")).toHaveText("Overridden")
})

test("the boost:frame-missing event following a link to a page without a matching frame can drive a Visit", async ({
  page
}) => {
  await page.locator("#missing").evaluate((frame) => {
    frame.addEventListener(
  ...
  await expect(page.locator("boost-frame#missing")).not.toBeAttached()

  await page.goBack()
  await nextEventNamed(page, "boost:load")

  await expect(page).toHaveURL(withPathname("/src/tests/fixtures/frames.html"))
  await expect(page.locator("boost-frame#missing")).toBeAttached()
})

test("following a link to a page with a matching frame does not dispatch a boost:frame-missing event", async ({
  page
}) => {
  await page.click("#link-frame")

  expect(await noNextEventNamed(page, "boost:frame-missing")).toBeTruthy()
  ...
  expect(
    src,
    "navigates frame without dispatching boost:frame-missing"
  ).toContain(
...
test("following a link within a frame which has a target set navigates the target frame without morphing even when frame[refresh=morph]", async ({ page }) => {
  ...
  expect(await nextEventOnTarget(page, "frame", "boost:before-frame-render")).toBeTruthy()
  expect(await noNextEventOnTarget(page, "frame", "boost:before-frame-morph")).toBeTruthy()
  await expect(page.locator("#frame h2")).toHaveText("Frame: Loaded")
})

test("navigating from within replaces the contents even with boost-frame[refresh=morph]", async ({ page }) => {
  await page.click("#add-refresh-morph-to-frame")
  await page.click("#link-frame")
  await nextBeat()
...
test("calling reload on a frame[refresh=morph] morphs the contents", async ({ page }) => {
  ...
  expect(await nextEventOnTarget(page, "frame", "boost:before-frame-render")).toBeTruthy()
  expect(await nextEventOnTarget(page, "frame", "boost:before-frame-morph")).toBeTruthy()
})
...
test("calling reload on a frame[refresh=morph] reloads descendant frame[refresh=morph]s via reload() as well", async ({ page }) => {
  ...
  expect(await nextEventOnTarget(page, "nested-root", "boost:before-frame-morph")).toBeTruthy()
  expect(await nextEventOnTarget(page, "nested-child", "boost:before-frame-morph")).toBeTruthy()
  expect(await noNextEventOnTarget(page, "nested-child-navigate-top", "boost:before-frame-morph")).toBeTruthy()
})

test("calling reload on a frame[refresh=morph] preserves [data-boost-permanent] elements", async ({ page }) => {
  await page.click("#add-src-to-frame")
  await page.click("#add-refresh-morph-to-frame")
...
test("following a link within a descendant frame whose ancestor declares a target set navigates the descendant frame", async ({
  ...
  const selector = "#nested-root[target=frame] #nested-child a:not([data-boost-frame])"
...
test("following a link that declares data-boost-frame within a frame whose ancestor respects the override", async ({
  page
}) => {
  await page.click("#nested-root[target=frame] #nested-child a[data-boost-frame]")
...
test("following a link within a frame with target=_top navigates the page", async ({ page }) => {
  ...
  await page.click("#navigate-top a:not([data-boost-frame])")
...
test("following a link that declares data-boost-frame='_self' within a frame with target=_top navigates the frame itself", async ({
  page
}) => {
  await expect(page.locator("#navigate-top")).not.toHaveAttribute("src")

  await page.click("#navigate-top a[data-boost-frame='_self']")
...
test("following a link with data-boost-frame='_parent' within a nested frame navigates the parent frame", async ({
  page
}) => {
...
test("submitting a form with data-boost-frame='_parent' within a nested frame navigates the parent frame", async ({
  page
}) => {
...
test("following a link with data-boost-frame='_parent' navigates only the immediate parent frame", async ({ page }) => {
...
test("following a link with data-boost-frame='_parent' in a top-level frame navigates the page", async ({ page }) => {
...
test("following a link with data-boost-frame='_parent' when parent frame is disabled navigates the page", async ({ page }) => {
...
test("following a link to a page with a <boost-frame recurse> which lazily loads a matching frame", async ({
  page
}) => {
  ...
  await nextEventOnTarget(page, "recursive", "boost:frame-load")
  await nextEventOnTarget(page, "composer", "boost:frame-load")

  await expect(page.locator("#recursive details")).not.toHaveAttribute("open")
})

test("submitting a form that redirects to a page with a <boost-frame recurse> which lazily loads a matching frame", async ({
  page
}) => {
  await page.click("#recursive summary")
...
test("removing [disabled] attribute from eager-loaded frame navigates it", async ({ page }) => {
  ...
  expect(
    await noNextEventOnTarget(page, "frame", "boost:before-fetch-request"),
    "[disabled] frames do not submit requests"
  ).toBeTruthy()

  await page.evaluate(() => document.getElementById("frame")?.removeAttribute("disabled"))

  await nextEventOnTarget(page, "frame", "boost:before-fetch-request")
})
...
test("evaluates frame script elements on each render", async ({ page }) => {
  ...
  await nextEventOnTarget(page, "body-script", "boost:frame-load")
...
test("redirecting in a form is still navigatable after redirect", async ({ page }) => {
  ...
  await nextEventOnTarget(page, "form-redirect", "boost:frame-load")
  await expect(page.locator("boost-frame#form-redirect h2")).toHaveText("Form Redirect")

  await page.click("#submit-form")
  await nextEventOnTarget(page, "form-redirect", "boost:frame-load")
  await expect(page.locator("boost-frame#form-redirect h2")).toHaveText("Form Redirected")
...
test("'boost:frame-render' is triggered after frame has finished rendering", async ({ page }) => {
  await page.click("#frame-part")

  await nextEventNamed(page, "boost:frame-render") // recursive
  const { fetchResponse } = await nextEventNamed(page, "boost:frame-render")

  expect(fetchResponse.response.url).toContain("/src/tests/fixtures/frames/part.html")
})

test("navigating a frame from an outer link with a boost-frame child fires events", async ({ page }) => {
  await page.click("#outside-frame-link-with-frame-child")

  await nextEventOnTarget(page, "frame", "boost:before-fetch-request")
  await nextEventOnTarget(page, "frame", "boost:before-fetch-response")
  const { fetchResponse } = await nextEventOnTarget(page, "frame", "boost:frame-render")
...
test("navigating a frame from an outer link fires events", async ({ page }) => {
  await listenForEventOnTarget(page, "outside-frame-form", "boost:click")
  await page.click("#outside-frame-form")

  await nextEventOnTarget(page, "outside-frame-form", "boost:click")
  await nextEventOnTarget(page, "frame", "boost:before-fetch-request")
  await nextEventOnTarget(page, "frame", "boost:before-fetch-response")
  const { fetchResponse } = await nextEventOnTarget(page, "frame", "boost:frame-render")
  expect(fetchResponse.response.url).toContain("/src/tests/fixtures/frames/form.html")
...
test("navigating a frame from an inner link fires events", async ({ page }) => {
  await listenForEventOnTarget(page, "link-frame", "boost:click")
  await page.click("#link-frame")

  await nextEventOnTarget(page, "link-frame", "boost:click")
  await nextEventOnTarget(page, "frame", "boost:before-fetch-request")
  await nextEventOnTarget(page, "frame", "boost:before-fetch-response")
  const { fetchResponse } = await nextEventOnTarget(page, "frame", "boost:frame-render")
...
test("navigating a frame targeting _top from an outer link fires events", async ({ page }) => {
  await listenForEventOnTarget(page, "outside-navigate-top-link", "boost:click")
  await page.click("#outside-navigate-top-link")

  await nextEventOnTarget(page, "outside-navigate-top-link", "boost:click")
  await nextEventOnTarget(page, "html", "boost:before-fetch-request")
  await nextEventOnTarget(page, "html", "boost:before-fetch-response")
  await nextEventOnTarget(page, "html", "boost:before-render")
  await nextEventOnTarget(page, "html", "boost:render")
  await nextEventOnTarget(page, "html", "boost:load")
...
test("invoking .reload() re-fetches the frame's content", async ({ page }) => {
  ...
  expect(
    ...
    [
      ["frame", "boost:before-fetch-request"],
      ["frame", "boost:before-fetch-response"],
      ["frame", "boost:before-frame-render"],
      ["frame", "boost:frame-render"],
      ["frame", "boost:frame-load"]
    ]
  )
})
...
test("navigating pushing URL state from a frame navigation fires events", async ({ page }) => {
  ...
  expect(
    await nextAttributeMutationNamed(page, "frame", "aria-busy"),
    "sets aria-busy on the <boost-frame>"
  ).toEqual(
    "true"
  )
  await nextEventOnTarget(page, "frame", "boost:before-fetch-request")
  ...
  await nextEventOnTarget(page, "frame", "boost:frame-render")
  await nextEventOnTarget(page, "frame", "boost:frame-load")
  expect(await nextAttributeMutationNamed(page, "frame", "aria-busy"), "removes aria-busy from the <boost-frame>").not.toBeTruthy()

  expect(await nextAttributeMutationNamed(page, "html", "aria-busy"), "sets aria-busy on the <html>").toEqual("true")
  await nextEventOnTarget(page, "html", "boost:before-visit")
  await nextEventOnTarget(page, "html", "boost:visit")
  await nextEventOnTarget(page, "html", "boost:before-render")
  await nextEventOnTarget(page, "html", "boost:render")
  await nextEventOnTarget(page, "html", "boost:load")
...
test("navigating a frame with a form[method=get] that does not redirect still updates the [src]", async ({
  ...
  expect(await noNextEventOnTarget(page, "frame", "boost:before-fetch-request")).toBeTruthy()
...
test("navigating boost-frame[data-boost-action=advance] from within pushes URL state", async ({ page }) => {
  await page.click("#add-boost-action-to-frame")
  await page.click("#link-frame")
  await nextEventNamed(page, "boost:load")
...
test("navigating boost-frame[data-boost-action=advance] from outside with target pushes URL state", async ({ page }) => {
  await page.click("#add-boost-action-to-frame")
  await page.click("#hello a")
  await nextEventNamed(page, "boost:load")
...
test("navigating boost-frame[data-boost-action=advance] with Boost.visit pushes URL state", async ({ page }) => {
  const path = "/src/tests/fixtures/frames/frame.html"

  await page.click("#add-boost-action-to-frame")
  await page.evaluate((path) => window.Boost.visit(path, { frame: "frame" }), path)
  await nextEventNamed(page, "boost:load")
...
test("navigating boost-frame without advance with Boost.visit specifying advance pushes URL state", async ({ page }) => {
  const path = "/src/tests/fixtures/frames/frame.html"

  await page.evaluate((path) => window.Boost.visit(path, { frame: "frame", action: "advance" }), path)
  await nextEventNamed(page, "boost:load")
...
test("navigating boost-frame[data-boost-action=advance] to the same URL clears the [aria-busy] state", async ({
  page
}) => {
  await page.click("#link-outside-frame-action-advance")
  await nextEventNamed(page, "boost:load")
  ...
  await expect(page.locator("#frame"), "clears boost-frame[aria-busy]").not.toHaveAttribute("aria-busy")
  await expect(page.locator("#html"), "clears html[aria-busy]").not.toHaveAttribute("aria-busy")
})

test("navigating a boost-frame with an a[data-boost-action=advance] preserves page state", async ({ page }) => {
...
test("a boost-frame that has been driven by a[data-boost-action] can be navigated normally", async ({ page }) => {
  ...
  expect(await noNextEventNamed(page, "boost:load")).toBeTruthy()
  await expect(page.locator("#hello h2")).toHaveText("Frames: #hello")
  await expect(page).toHaveURL(withPathname("/src/tests/fixtures/frames/hello.html"))
})

test("navigating boost-frame from within with a[data-boost-action=advance] pushes URL state", async ({ page }) => {
...
test("navigating frame with a[data-boost-action=advance] pushes URL state", async ({ page }) => {
  await page.click("#link-outside-frame-action-advance")
  await nextEventNamed(page, "boost:load")
...
test("navigating frame with form[method=get][data-boost-action=advance] pushes URL state", async ({ page }) => {
...
test("navigating frame with form[method=get][data-boost-action=advance] to the same URL clears the [aria-busy] state", async ({
  page
}) => {
  await page.click("#form-get-frame-action-advance button")
  await nextEventNamed(page, "boost:load")
...
test("navigating frame with form[method=post][data-boost-action=advance] pushes URL state", async ({ page }) => {
...
test("navigating frame with form[method=post][data-boost-action=advance] to the same URL clears the [aria-busy] state", async ({
  page
}) => {
  await page.click("#form-post-frame-action-advance button")
  await nextEventNamed(page, "boost:load")
...
test("navigating frame with button[data-boost-action=advance] pushes URL state", async ({ page }) => {
...
test("navigating back after pushing URL state from a boost-frame[data-boost-action=advance] re-fetches the page", async ({
  page
}) => {
  await page.click("#add-boost-action-to-frame")
  await page.click("#link-frame")
...
test("navigating back then forward after pushing URL state from a boost-frame[data-boost-action=advance] re-fetches the frame URL as a page", async ({
  page
}) => {
  await page.click("#add-boost-action-to-frame")
  await page.click("#link-frame")
...
test("boost:before-fetch-request fires on the frame element", async ({ page }) => {
  await page.click("#hello a")
  expect(await nextEventOnTarget(page, "frame", "boost:before-fetch-request")).toBeTruthy()
})

test("boost:before-fetch-response fires on the frame element", async ({ page }) => {
  await page.click("#hello a")
  expect(await nextEventOnTarget(page, "frame", "boost:before-fetch-response")).toBeTruthy()
})
...
test("navigating a eager frame with a link[method=get] that does not fetch eager frame twice", async ({
  ...
  const fetchLogs = eventLogs.filter(
    ([name, options]) =>
      name == "boost:before-fetch-request" && options?.url?.includes("/src/tests/fixtures/frames/frame_for_eager.html")
  )
